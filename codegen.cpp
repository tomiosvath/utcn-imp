...
{
  auto entry = MakeLabel();
  auto else_label = MakeLabel();
  auto exit = MakeLabel();

  EmitLabel(entry);
  LowerExpr(scope, ifStmt.GetCond());
  EmitJumpFalse(else_label);
  LowerStmt(scope, ifStmt.GetStmt());
  EmitJump(exit);
  EmitLabel(else_label);
  if(auto else_branch = ifStmt.GetElseStmt()){
    LowerStmt(scope, *else_branch);
  }
  EmitLabel(exit);
}
...
void Codegen::LowerExpr(const Scope &scope, const Expr &expr)
{
  switch (expr.GetKind()) {
    ...
    case Expr::Kind::INT: {
      EmitPush(static_cast<const IntExpr &>(expr).GetNumber());
    }
  }
}
...
// -----------------------------------------------------------------------------
void Codegen::EmitPush(uint64_t value)
{
  depth_ += 1;
